...
import { ArrowLeft, Database, ExternalLink, RefreshCw, Settings, AlertCircle, Calendar } from 'lucide-react';
    ...
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/notion-database-query`,
        {
          ...
          body: JSON.stringify({ db_type: 'chunk' }),
        }
      );
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            ...
            {!loading && !isNotConfigured && !error && (
              <button
                onClick={fetchData}
                className="p-2 hover:bg-gray-100 rounded-lg transition-all duration-200 text-gray-500 hover:text-blue-600"
              >
        ...
        {!loading && isNotConfigured && (
          <div className="bg-white rounded-2xl shadow-sm border border-amber-200 p-10 text-center max-w-md mx-auto mt-8">
            ...
            <p className="text-gray-500 text-sm leading-relaxed">
              設定画面でNotion APIキーと<br />チャンクDB IDを登録してください。
            </p>
          </div>
        )}
        ...
        {!loading && !error && items.length > 0 && (
          <div className="grid gap-3">
            {items.map((item) => (
              <div
                key={item.id}
                className="bg-white rounded-xl border border-gray-100 hover:border-blue-200 hover:shadow-md transition-all duration-200"
              >
                <div className="p-5">
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <a
                      ...
                      className="flex-shrink-0 p-2 text-gray-300 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-all duration-200"
                    >
